...
for n in $names
do
grep -A 1 ">lcl|$n" $readin_base.fix | tail -n +2 | awk '{print length($1)}' >> $repname.rdlen
done
...
then
grep -A 1 ">lcl|$nu" $readin_base.fix >> rc.tmp.fasta
else 
grep -A 1 ">lcl|$nu" $readin_base.fix >> norc.tmp.fasta
fi
done
#wc -l rc.tmp.fasta
#wc -l norc.tmp.fasta
